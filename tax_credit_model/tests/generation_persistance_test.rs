...
use tax_credit_model_server::{
    files::file_system::{
        file_metadata, write_file, CreateMode, File, Permissions, ReadMode, WriteMode,
    },
    persistance::generation::{DiskGenerationPersistanceClient, GenerationClient},
    schema::{
...
fn list_generations_from_disk() {
    ...
    let file = File::new(
        &TempDirectory::canonicalize_path(&directory, "generations.data"),
        &Permissions::writeable(CreateMode::CreateOrRead),
    );
    let client = DiskGenerationPersistanceClient::new(&TempDirectory::canonicalize_path(
        &directory,
        "generations.data",
    ))
    .unwrap();
...
fn create_generations_on_disk() {
    ...
    let client = DiskGenerationPersistanceClient::new(file.path()).unwrap();
